...
function consultTicket() {
    ...
    if (consultationModal) {
        ...
        setTimeout(() => {
            const input = document.getElementById('previousTicketNumber');
            if (input) {
                input.focus();
            }
        }, 300);
...
function searchAndCopyTicket() {
    const ticketNumber = document.getElementById('previousTicketNumber').value.trim();
    
    if (!ticketNumber) {
        showNotification('Veuillez saisir un numéro de référence valide.', 'error');
        return;
    }
    
    // Validate ticket number format (basic validation)
    if (ticketNumber.length < 8) {
        showNotification('Le numéro de référence doit contenir au moins 8 caractères.', 'error');
        return;
    }
    
    // Show loading state
    const searchBtn = document.querySelector('.consultation-btn');
    ...
    setTimeout(() => {
        ...
        showNotification('E-Ticket trouvé! Les informations ont été copiées.', 'success');
        ...
        setTimeout(() => {
            ...
            // Redirect to step 1 form with copied data
            window.location.href = '1 class.html';
        }, 1500);
